...
def get_ticker_missing(
    ...
    eligible_tickers_available_data = ticker_map.merge(
        tickers_available_data.reset_index(), on="bloomberg_ticker", how="left"
    )
    ...
    tickers_outdated["start"] = (tickers_outdated["date_max"] + pd.DateOffset(1)).dt.strftime(
        "%Y-%m-%d"
    )
...
def get_data(
    ...
    ticker_data = ticker_data[
        ticker_data.bloomberg_ticker.isin(ticker_universe["bloomberg_ticker"])
    ]
    ...
    for features_generator in features_generators:
        ticker_data, feature_names_aux = features_generator.generate_features(
            ticker_data, feature_prefix
        )
        feature_names.extend(feature_names_aux)

    # merge our feature data with Numerai targets
    ml_data = pd.merge(ticker_data, targets, on=["date", "bloomberg_ticker"], how="left")
...
def download_ticker(ticker, start_epoch, end_epoch):
    ...
    while tries > 0:
        tries -= 1
        try:
            data = requests.get(url=url, params=params, headers={"User-Agent": user_agent})
            data_json = data.json()
