...
class VarChange:
    ...
    def generate_features(self, ticker_data):
        logger.info(f"generating var change {self.num_days} " f"for {self.variable}...")
...
class RSI:
    ...
    def get_feature_names(self, prefix_name):
        ...
        feat_quintile_lag = {step: f"{prefix_name}_quintile_lag_{step}" for step in self.steps}
        feat_rsi_diff = {step: f"{prefix_name}_diff_{step}" for step in self.steps[:-1]}
        feat_rsi_diff_abs = {step: f"{prefix_name}_abs_diff_{step}" for step in self.steps[:-1]}
    ...
    def generate_features(self, ticker_data, feature_prefix=None):
        ...
        (feat_quintile_lag, feat_rsi_diff, feat_rsi_diff_abs) = self.get_feature_names(
            feature_prefix_name
        )
        ...
        for day in self.steps:
            col = f"{feature_prefix_name}_quintile"
            ticker_data[feat_quintile_lag[day]] = (
                ticker_groups[col].transform(lambda group: group.shift(day)).astype("float16")
            )
        ...
        for i in range(len(self.steps) - 1):
            ...
            ticker_data[feat_rsi_diff_abs[step]] = np.abs(ticker_data[feat_rsi_diff[step]])

        feature_names = (
            list(feat_quintile_lag.values())
...
class SMA:
    ...
    def generate_features(self, ticker_data, feature_prefix=None):
        ...
        (feat_quintile_lag, feat_sma_diff, feat_sma_diff_abs) = self.get_feature_names(
            feature_prefix_name
        )
        ...
        for i in range(len(self.steps) - 1):
            ...
            ticker_data[feat_sma_diff_abs[self.steps[i]]] = np.abs(
                ticker_data[feat_sma_diff[self.steps[i]]]
            )
